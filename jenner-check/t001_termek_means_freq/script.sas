/* Adapted from adatgeneralas.sas (TERMEK block) + feladat_megoldasok.sas (feladat 2).
   Generates the TERMEK (window/door product catalog) dataset exactly as in
   adatgeneralas.sas, then reproduces the student's solution to feladat 2:
   compute product area (terul = szele*hossza) and run elementary statistics
   (PROC MEANS) plus frequency tables (PROC FREQ) on the nomenclature columns. */
